...
fun produce(data: Array<RawData>, @DesignOnlyType() acc: OpenNode): Node =                                     
  data match {
    ...
    case [ x ~ xs ] -> do {
        x.kind match {
          case is "DW.PRE_EXECUTION" | "DW.START_READING" | "DW.START_WRITING"  -> do {
              var newNode = {
                  name:                        
          ...
          case is "DW.POST_EXECUTION" | "DW.END_READING" | "DW.END_WRITING" -> 
            if(acc.id == x.id) do {
...
fun mergeNode(n:Null) = null

fun mergeNode(@DesignOnlyType() node: Node) = do {

    fun mergeNodes(nodes: Array<Node>) = do {
        nodes
